...
Planned rotation publishes the new public key and `keys/transitions/<new-key-id>.json` before any manifest uses it. The transition has strict top-level `schemaVersion`, `signed`, and `signature` objects. Its signed payload contains `fromKeyId`, `toKeyId`, `toPublicKeySha256` (the canonical new key document digest), and `authorizedAt`; its Ed25519 envelope is signed by `fromKeyId`, which MUST already exist in the trusted base branch. CI rejects a newly added key without this authorization. Public-key documents and transition records become immutable when merged, and CI revalidates existing transitions against their authorized digest. Consumers accept the new key only after validating that transition or receiving the key in a trusted Gajae Code trust-store update.

After the overlap window, repository revocation adds immutable `keys/revocations/<revoked-key-id>.json`. Its strict signed payload contains `keyId`, UTC `revokedAt`, `replacementKeyId` (or `null`), and reason `compromise` or `retired`; a different active Ed25519 key signs the envelope. CI authenticates the record, rejects authorization by a key already revoked at that time, preserves validation of historical manifests published before `revokedAt`, and rejects `latest.json` whenever its key has any effective revocation record. When no other trusted key remains, revocation MUST be delivered in a trusted Gajae Code trust-store update. Existing key, transition, and revocation files are never rewritten.
